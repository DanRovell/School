...
int main() 
{
    ...
    printf("Enter the number of processes: ");//asks for number of processes
    scanf("%d", &p);

    for (i = 0; i < p; i++) 
	{
        running[i] = 1;							//running is given a 1(aka true) to all spots
        count++;								//count is the number of processes
    }
 
    printf("Enter maximum for each resource: ");// this is the max number of each resource, i.e. first is 10,
	for (i = 0; i < r; i++)						  // second is 5, third is 7, etc..
	{
        scanf("%d", &maxResources[i]);
	}
 
    printf("Enter Allocation matrix: ");// user enter the Allocation matrix
    for (i = 0; i < p; i++) 
	{
        for (j = 0; j < r; j++)
		{
    ...
    printf("Enter Max matrix: ");//user enter the Max matrix
    for (i = 0; i < p; i++) 
	{
        for (j = 0; j < r; j++)
		{

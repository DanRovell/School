...
def findFile(data, head, foot):
    ...
    return found, index2
def findFile2(data, head, foot, temp):
    print len(data)
    for i in range(temp, len(data)): #For loop to find the header
        x = data[i:i+(len(head))]
        if x == head:
            index1 = i
            break
...
JPEG,temp = findFile(data, jpegHead, jpegFoot)
JPEG2,temp = findFile2(data, jpegHead, jpegFoot, temp)
